...
def migrate_db():
    ...
    if "address" not in user_columns:
        print("Migrating users table: adding address column")
        with engine.begin() as connection:
            connection.execute(text("ALTER TABLE users ADD COLUMN address TEXT NULL"))
    ...
    if "photoprofil" not in user_columns:
        ...
        with engine.begin() as connection:
            connection.execute(text("ALTER TABLE users ADD COLUMN photoprofil VARCHAR(255) NULL"))
